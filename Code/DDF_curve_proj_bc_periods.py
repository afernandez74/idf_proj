...
if scenario == 'ssp370':
    data_futu = data_futu.where(data_futu['model'] != 'BCC-CSM2-MR', drop = True)
...
for period, da in data_loc.groupby('period'):
    da = da.sortby('RI')
    for RI_i, da_2 in da.groupby('RI'):
        mean.append(da_2.median(dim = 'source').values)
    ...
    dat_y_2 = q25
    dat_y_3 = q75
    plt.plot(dat_x, dat_y_1, label = period, linewidth=3, color = colors[i],zorder = 2)
...
ax.plot(dat_x,a14[0], label = 'NA14', color = 'black', linewidth = 3,zorder = .5)
...
dat_y_2 = q25
dat_y_3 = q75
plt.plot(dat_x, dat_y_1, label = 'Historical', linewidth=3, color = 'green',zorder = 1)
...
if save_option == 'y':
    save_path_name = save_path+'DDF_curve_'+f'{scenario}_{D}da_{site}'
    # Save as SVG
    plt.savefig(save_path_name +'.svg', format='svg', dpi=300, bbox_inches='tight')
    # Save as PNG
    plt.savefig(save_path_name +'.png', format='png', dpi=300, bbox_inches='tight')
...
for i in range (len(lat_lons)):    
    x,y = transformer.transform(lat_lons[i][1],lat_lons[i][0])
    ax.plot(x,y,'ro',markersize = 10,label=f'{sites[i]}')
