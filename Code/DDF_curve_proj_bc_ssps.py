...
while True:
    try:
        ix = int(input("Enter the number of your future period choice: "))
        if 0 <= ix <= len(periods):
            break
        else:
...
# =============================================================================
strings = [period, f'_{bc_source}', f'{D:02}da']
...
for scenario, da in data_loc.groupby('scenario'):
    da = da.sortby('RI')
    ...
    dat_y_2 = q25
    dat_y_3 = q75
    plt.plot(dat_x, dat_y_1, label = scenario, linewidth=3, linestyle= '--', color = colors[i],zorder = 2)
    # plt.plot(dat_x, dat_y_2, linewidth=.1, color = colors[i], linestyle = '--',zorder = .1)
    # plt.plot(dat_x, dat_y_3, linewidth=.1, color = colors[i], linestyle = '--',zorder = .1) 
    if scenario == 'ssp370':
        plt.plot(dat_x,dat_y_1, linewidth=3,marker='o',color = colors[i])
        ax.fill_between(dat_x, dat_y_2, dat_y_3, color=colors[i], alpha=0.15,zorder = .5)
    mean = []
    q25 =[]
    q75 = []
    i=i+1
ax.plot(dat_x,a14[0], label = 'NA14', color = 'black', marker = 'o', linewidth = 3,zorder = .5)
# ax.plot(dat_x,a14[1], color = 'grey', linewidth = 0.5,zorder = .1)
# ax.plot(dat_x,a14[2], color = 'grey', linewidth = 0.5,zorder = .1)
ax.fill_between(dat_x, a14[1], a14[2], color='black', alpha=0.1,zorder = .1,hatch='|')
...
dat_y_2 = q25
dat_y_3 = q75
plt.plot(dat_x, dat_y_1, label = 'Historical', linewidth=3, color = 'forestgreen',zorder = 1)
# plt.plot(dat_x, dat_y_2, linewidth=.5, color = 'red', linestyle = '--',zorder = .1)
# plt.plot(dat_x, dat_y_3, linewidth=.5, color = 'red', linestyle = '--',zorder = .1)  
ax.fill_between(dat_x, dat_y_2, dat_y_3, color = 'green', alpha=alpha_fill,zorder = .2)
...
#save path results
save_path = '../Figures/abs_proj/mean_model_abs_IDF_curves_ssps/'

if save_option == 'y':
    save_path_name = save_path+'DDF_curve_'+f'{period}_{D}da_{site}'
    # Save as SVG
...
#%%
# #%% olot model spread for ssp 370

# fig,ax = plt.subplots()

# data_loc_scenario = data_loc.where(data_loc.scenario=='ssp370',drop=True)
# for model, da in data_loc_scenario.groupby('model'):
#     da = da.sortby('RI')
#     dat_x = da.RI.values
#     dat_y = da.values
#     plt.plot(dat_x, dat_y,label = model, linewidth=1.5)

# # data_loc_scenario_mean = data_loc_scenario.mean(dim='model')
...
for (lat, lon), site in zip(lat_lons, sites):
    ax.plot(lon, lat,
            marker='o',
...
for (lat, lon), site in zip(lat_lons, sites):
    ax.text(lon + 0.1, lat + 0.1, site, fontsize=8, color='black',
            transform=ccrs.PlateCarree(), zorder=5)
save_option = input("Save figure? (y/n): ").lower()

#save path results
save_path = '../Figures/abs_proj/sites_map/'

if save_option == 'y':
    save_path_name = save_path+'Sites_map'
    # Save as SVG
    plt.savefig(save_path_name +'.svg', format='svg', dpi=300, bbox_inches='tight')
    # Save as PNG
    plt.savefig(save_path_name +'.png', format='png', dpi=300, bbox_inches='tight')
